...
bool ssd1306_read(ssd1306_t* config, uint8_t *datatx, size_t datatx_len, uint8_t *datarx, size_t datarx_len) {
    return construct_i2c_read_data(config->i2c_mod, SSD1306_I2C_ADR, datatx, datatx_len, datarx, datarx_len);
}


// ======================= CALLABLE FUNCTIONS =======================
bool ssd1306_init(ssd1306_t* config) {
    if(!config->i2c_mod->init_done) {
        init_i2c_module(config->i2c_mod);
    }
    ...
    ssd1306_send_cmd_list(config, cmds, count_of(cmds));
...
void ssd1306_render(ssd1306_t* config, uint8_t *buf, render_area_t *area) {
    // update a portion of the display with a ssd1306_render area
    uint8_t cmds[] = {
    ...
    ssd1306_send_cmd_list(config, cmds, count_of(cmds));
    ssd1306_send_buffer(config, buf, ssd1306_calculate_render_arena_buflen(area));
}
